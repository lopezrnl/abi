...
import { upload } from "@vercel/blob/client"; // Added client upload helper
...
const Diary = () => {
  ...
  const saveEntry = async (e) => {
    ...
    try {
      ...
      // Improved: Use @vercel/blob client-side upload helper
      for (const file of imageFiles) {
        const newBlob = await upload(file.name, file, {
          access: 'public',
          handleUploadUrl: '/api/upload', // This points to your serverless token generator
        });
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-6 lg:p-10">
      ...
      {isAdding && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-stone-900/40 backdrop-blur-sm">
          <form onSubmit={saveEntry} className="bg-white w-full max-w-2xl p-6 md:p-8 rounded-[1.5rem] md:rounded-[2rem] space-y-4 shadow-2xl relative max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center">
              <span className="font-serif italic text-stone-500 text-sm">New Memory Entry</span>
            ...
            <div className="flex flex-col md:grid md:grid-cols-2 gap-4">
              ...
              <select value={tag} onChange={e => setTag(e.target.value)} className="bg-stone-50 p-2.5 rounded-xl text-xs font-medium outline-none border border-stone-100">
            ...
            <textarea className="w-full h-32 outline-none text-stone-600 italic bg-stone-50/30 p-4 rounded-2xl border border-stone-100 text-sm" placeholder="What happened today?" value={content} onChange={e => setContent(e.target.value)} required />
            
            <div className="bg-rose-50/30 p-4 md:p-6 rounded-2xl border-2 border-dashed border-rose-100">
            ...
            <button 
              ...
              className="w-full bg-stone-800 text-white py-4 rounded-2xl text-[10px] font-bold uppercase tracking-widest shadow-lg hover:bg-rose-600 transition-colors disabled:bg-stone-400"
            >
      ...
      {selectedMemory && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-2 sm:p-4 bg-stone-900/60 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-4xl rounded-[1.5rem] md:rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col md:flex-row max-h-[95vh] md:max-h-[85vh]">
            <div className="w-full md:w-1/2 bg-stone-100 h-64 md:h-auto overflow-y-auto p-3 md:p-4 space-y-4">
            ...
            <div className="w-full md:w-1/2 p-6 md:p-10 flex flex-col relative bg-white overflow-y-auto">
              <button onClick={() => setSelectedMemory(null)} className="absolute top-4 right-4 text-stone-300 hover:text-stone-800 text-xl p-2">✕</button>
              <div className="mt-4">
                <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-rose-400 bg-rose-50 px-4 py-1.5 rounded-full">{selectedMemory.tag}</span>
                <h2 className="text-2xl md:text-4xl font-serif italic text-stone-800 mt-4 md:mt-6 leading-tight">{selectedMemory.title}</h2>
                <p className="text-stone-400 text-[10px] mt-2 uppercase tracking-widest">Archived on {selectedMemory.date}</p>
                <div className="h-px w-10 bg-rose-200 my-6"></div>
                <p className="text-stone-600 leading-relaxed font-serif italic text-base whitespace-pre-wrap">"{selectedMemory.content}"</p>
              </div>
              <button onClick={() => setSelectedMemory(null)} className="mt-8 md:mt-auto w-full border border-stone-100 py-3 rounded-xl text-[9px] font-bold uppercase tracking-widest hover:bg-stone-50">Close Gallery</button>
            </div>
          </div>
        </div>
      )}
